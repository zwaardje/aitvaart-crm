"use client";

import React from "react";
import { GenericCard } from "@/components/ui/GenericCard";
import { useCurrentUserOrganization } from "@/hooks/useOrganizations";
import { OrganizationNoteForm } from "@/components/forms/OrganizationNoteForm";

export default function WishIntakePage() {
  const { data, refetch } = useCurrentUserOrganization();
  const organization = data?.organization;

  if (!organization) return null;

  return (
    <div className="flex flex-col gap-4">
      <GenericCard
        title="Wensen- en intakegesprekken"
        content={
          <div className="text-sm whitespace-pre-wrap">
            {organization.wish_intake_notes || "Nog geen informatie opgegeven"}
          </div>
        }
        actions={
          <OrganizationNoteForm
            organizationId={organization.id}
            column="wish_intake_notes"
            title="Wensen- en intakegesprekken"
            defaultValue={organization.wish_intake_notes as any}
            onSuccess={refetch}
          />
        }
      />
    </div>
  );
}
